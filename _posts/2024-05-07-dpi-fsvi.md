...
In information theory, the **data processing inequality (DPI)** expresses a fundamental idea: processing data stochastically cannot increase information. The DPI examines how information flows and transforms through stochastic processes, governing what reductions or bottlenecks may emerge.  

In this blog post, we will first study the DPI, developing intuition through vivid examples and detailed proofs---especially the equality case, which is arguably the best way to understand inequalities. We will consider classic forms of the DPI as well as DPIs relating probability distributions more broadly.

Then, we explore the intriguing connection between DPI and **function-space variational inference (FSVI)**, a modern Bayesian deep learning technique that focuses on the Bayesian predictive posterior rather than the parameter space.
We apply the DPI to recover several interesting results from the literature in a simple form and build intuitions for the relationship between parameter and functional priors. 
...
think of parameter symmetries like different paths leading to the same destination; they might look different but end up at the same predictions<d-footnote>Thanks to ChatGPT for this analogy! 🤗</d-footnote>.

As a nice example and application, we relate FSVI to training with knowledge distillation and label entropy regularization: potentially more meaningful priors than the ones usually used in Bayesian neural networks<d-footnote>In many papers, an isotropic Gaussian is used because of its simplicity. Indeed, there are better alternatives, see Fortuin et al (2022)<d-cite key="fortuin2022bayesian"></d-cite> and Fortuin (2022)<d-cite key="fortuin2022priors"></d-cite>.</d-footnote>.
...
The data processing inequality examines how information cannot increase due to processing. In information theory, it is usually stated based on a Markov chain of random variables $$X \rightarrow Y \rightarrow Z$$ and their mutual information. We will look at different data processing inequalities that relate different distributions instead of different random variables. However, the blog posts in particular looks at the DPI when formulated using Kullback-Leibler (KL/🥬) divergences between distributions.

Concretely, this DPI states that processing data stochastically can only reduce information. More formally:
...
So far, we explored the data processing inequality and its applications in information theory. 

Specifically, we provided derivations and explanations for both the original data processing inequality as well as the 🥬 data processing inequality and related DPIs, emphasizing their significance and limitations. The examples and counterexample hopefully demonstrate the nuances of applying these inequalities in different contexts.

By understanding the foundations and limitations of the data processing inequality, we can leverage its insights to understand function-space variational inference.
...
<aside class="l-body box-note" markdown="1">
For more on the specific differences between marginal and joint objectives, the workshop paper by Kirsch et al (2022)<d-cite key="kirsch2022marginal"></d-cite> provides conceptual thoughts in the same light, which builds on the following important works:
...
In this blog post, we have explored the data processing inequality and its applications in information theory and Bayesian deep learning. We have looked at its meaning and intuitions for it in detail and have derived related versions.

In particular, we have seen that the data processing inequality provides an elegant perspective on why optimizing the variational posterior in function-space can be an effective inference strategy. 
Thus, this basic tool from information theory has helped us with a very simple deduction of this modern Bayesian deep learning method. It is reassuring that fundamental principles prove their enduring ability to explain recent advances.
...
Lastly, we have seen that the DPI can be used to derive a new perspective on label entropy regularization, which is a commonly used regularization technique in deep learning, and point out how it differs from knowledge distillation.
