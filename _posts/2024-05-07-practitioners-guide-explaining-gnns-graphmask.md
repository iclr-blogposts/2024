...
We cover why these considerations arise and how to navigate them practically.
...
Faithfulness is usefully thought of as a scale rather than a binary property.
As we will discuss, design choices either increase or decrease the faithfulness of an explanation.
These choices come with a trade-off: strict requirements for faithfulness are challenging to meet, as useful explanations are simplifications of the model that lose information (Jacovi and Goldberg, 2020 <d-cite key="jacovi-goldberg-2020-towards"></d-cite>).

GraphMask generates explanations by changing the model's input, in contrast to other methods that analyse the model when it is given 'normal' instances from a dataset.
These other methods yield less faithful explanations than GraphMask.
Methods that analyse gradients may not be reliable, as features with supposedly low importance may actually still be being used by the model <d-cite key="pmlr-v80-nie18a"></d-cite>.
It is additionally unclear whether attention in GNNs can be used as a measure of salience: evidence suggests that attention scores may be being used to scale messages in a way that is independent of their importance <d-cite key="schlichtkrull_interpreting_2021"></d-cite>.
...
### Official code
We recommend using the official GraphMask code, as third-party implementations we reviewed were incomplete at the time of writing (December 2023).
...
These modules implement their own functionality and add particular functions so GraphMask can interact with them.
...
The loss also increases when there is a <span style="color:#56B4E9">divergence</span> between the behaviour of the <span style="color:#009E73">target model using the original graph</span> and <span style="color:#0072B2">using the graph with some edges masked out</span>.
...
The loss function must include both objectives because they incentivise the model to do different things.
The first objective could easily be met by dropping all edges in the input, but this would likely substantially change the model's output.
...
In particular, the optimiser's attempted maximisation of the Lagrange multiplier $\lambda$ implements the constraint.
...
As previously mentioned, the masking must minimally change model behaviour for explanations to be faithful.
...
Predicting which edges can be masked at all layers is a difficult task, so breaking it down likely helps the initial stages of training in particular.
...
We found that activating BatchNorm during GraphMask training was unhelpful, for example, so we recommend deactivating it.
...
The authors therefore also compare the model's behaviour on its task pre- and post-edge masking, and only accept a trained probe if the deviation is small. 
...
The trade-off here is accepting fewer trained probes to increase faithfulness.
Our acceptance rate dropped from 90% to 10% on around 1500 probes fitted for 80 different target models, although raising the threshold to 7.5% changed questions and would have resulted in 55% probe acceptance.
In addition, we could not train acceptable probes for some target models, even after extensive hyperparameter optimisation.
...
We found that training was sensitive to the choice of hyperparameters, particularly when using the stricter requirement on model behaviour change described above.
The low yield means that a potentially time-consuming hyperparameter tuning process is needed.
We found that tuning is not easily transferrable between different target models, even when they only differ by random seed.
Configurations that flipped the fewest answers for some target models also changed a large proportion for others.
However, we noticed that the seed choice for probe training was particularly influential.
...
Even with the goal of minimising the proportion of retained edges, it is difficult to evaluate the precision of the resulting explanations.
By extension, it is also difficult to know when to stop hyperparameter optimisation and accept an explanation.
Precision cannot be measured, as for practical tasks the true explanation is unknown. 
We recommend following the authors in using an agreement metric like Fleiss' Kappa to evaluate when explanations with some proportion of retained edges should be accepted as high precision.
...
Integrating custom GNNs with the official GraphMask code to train probes and produce high-quality explanations is relatively straightforward.
We have discussed two important considerations for this training.
First, a suitable measure must be used to verify that the model behaviour does not change too much after some edges have been masked out.
The method used in the original paper may be too permissive, and we suggest a stricter measure to increase faithfulness.
Second, the high recall, unknown precision explanations require processing to ensure that they do not include too many spurious edges, which are actually not needed by the GNN.
We advocate extending the authors' use of Fleiss' Kappa as a proxy measure for precision.
