...
Broadly, algorthmic reasoning <d-cite key="velickovic2020neural"></d-cite> wants to study how well neural networks can learn to execute classical computer science algorithms. In particular to measure how well an algorithm has been learned we look at size-generalisation, i.e. if we train on inputs of size $$N$$ how well will the Neural Network perform on inputs of size $$2N$$ or $$10N$$. The idea is that neural networks often learn shortcuts that work well in-distribution, but fail out-of-distribution, whereas classical computer science algorithms work no matter the input size. The purpose of this exercise to study the generalisation of reasoning tasks, especially what tricks help to improve robustness and get the network closer to deducing logically rather than relying on statistical short cuts.
...
Traditionally the training approach has been teacher-forcing. In teacher forcing we train each step of the algorithm independently by feeding the network the ground-truth $$x_t$$ and computing the loss against $$y_t$$ at all $$t$$ simultaneously. This requires us to know the exact number of steps in the algorithm a priori, which is unrealistic in practice. While a termination network is suggested in <d-cite key="velickovic2020neural"></d-cite>, the issue is ignored in later papers such as <d-cite key="ibarz2022generalist"></d-cite>.
...
One approach to training neural networks that run until they reach a fixed point is deep equilibrium models<d-cite key="bai2019deep"></d-cite>. We discuss this approach next. As shown in <d-cite key="bai2019deep"></d-cite><d-cite key="baiblog"></d-cite>

$$z^* = f(z^*,x)$$

In backprop, we ultimately want to compute 

$$ \left(\frac{\partial z^*(.)}{\partial(.)}\right)^{\top} y$$
...
$$ \left(\frac{\partial z^*(.)}{\partial(.)}\right)^{\top} y = \left(\frac{\partial f(z^*, x)}{\partial (.)}\right)^{\top}\left(I-\frac{\partial f(z^*, x)}{\partial z^*}\right)^{-\top}y$$
...
$$g = \left(\frac{\partial f(z^*, x)}{\partial z^*}\right)^{-\top}g +y$$
