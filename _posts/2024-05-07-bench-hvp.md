...
toc:
  ...
  - name: A quick detour of automatic differentiation
  - subsections:
...
For such a function, standard AD can be used to efficiently compute the gradient of the loss $$\nabla f(\theta) = \left[ \frac{\partial f}{\partial \theta_i}(\theta)\right]_{1\le i \le d} \in \mathbb{R}^d$$, using the so-called backpropagation.
...
To compute the iHVP, many methods proposed in the literature rely on HVPs to solve \eqref{eq:linear_system} without ever accessing $$\nabla^2 f$$, like Neumann iterates<d-cite key="Ghadimi2018"></d-cite><d-cite key="Ji2021stocbio"></d-cite>, the Conjugate Gradient method or gradient descent steps<d-cite key="Arbel2022amigo"></d-cite><d-cite key="Dagreou2022SABA"></d-cite> in the quadratic form $$v\mapsto \frac12\langle\nabla^2f(\theta)v, v\rangle - \langle b, v\rangle$$. These methods are based on the computation of HVPs, as illustrated with the highlighted terms in the Conjugate Gradient method<d-cite key=Hestenes1952CG></d-cite><d-cite key=Nocedal2006></d-cite>. Thus, an efficient implementation of HVPs is crucial for the overall algorithm performance.
...
## A quick detour of automatic differentiation

Automatic differentiation is an important tool to compute exactly the derivatives of differentiable functions obtained as the composition of simple operations.
...
A key ingredient of AD is a computational graph associated with the code that evaluates a function. It is a directed acyclic graph that represents the succession of elementary operations required the evaluate a function.  Let us consider a multilayer linear perceptron (MLP) with 2 layers $$f_x:\mathbb{R}^2\times \mathbb{R}^{2\times 2}\to \mathbb{R}$$ defined for a provided $$x\in\mathbb{R}^2$$ by
...
In this graph, the vertices $$z_i$$ represent the intermediate states of the evaluation of $$f_x$$. Hence, the complexity of a function evaluation depends on the complexity of the considered computational graph. In terms of memory, to get the vertex $$z_i$$, we use the values stored by its parents in the graph. Thus, the memory footprint of the evaluation of the function is linked to the maximum number of parents that can have a vertex in the computational graph. Particularly in our case, each node has at most two parents. The Python code to compute $$f_x$$ by following the previous computational graph is the following
...
Let us consider a function $$f:\mathbb{R}^d\to\mathbb{R}^p$$ with this simple computational graph.

{% include figure.html path="assets/img/2024-05-07-bench-hvp/direct_graph.png" class="img-fluid"%}

By denoting $$z_1 = \theta$$ and $$z_n = f(\theta)$$ and assuming $$z_i\in\mathbb{R}^{m_i}$$, the chain rule gives us the expression of the Jacobian $$\frac{\partial f}{\partial \theta}(\theta)$$ as a product of the Jacobian of the intermediate states $$z_1, \dots, z_n$$.
\begin{equation}\label{eq:chain_rule}
  \underbrace{\frac{\partial f}{\partial \theta}(\theta)}\_{p\times q} = \frac{\partial z_n}{\partial z_1}
    =\frac{\partial z_n}{\partial z_2}\frac{\partial z_2}{\partial z_1}=\cdots = \underbrace{\frac{\partial z_n}{\partial z_{n-1}}}\_{m_n\times m_{n-1}}\underbrace{\frac{\partial z_{n-1}}{\partial z_{n-2}}}\_{m_{n-1}\times m_{n-2}}\cdots\underbrace{\frac{\partial z_2}{\partial z_1}}\_{m_2\times m_{1}}\enspace.
\end{equation}

Depending on the order of the multiplication, one can compute the derivative of $$f$$ with respect to $$z_1$$ in two ways: the forward mode and the reverse mode.

### Forward mode

The forward mode aims at computing JVPs 

\begin{equation}\label{eq:chain_rule_jvp}
  \frac{\partial f}{\partial \theta}(\theta)\times v = \frac{\partial z_n}{\partial z_{n-1}}\frac{\partial z_{n-1}}{\partial z_{n-2}}\cdots\frac{\partial z_2}{\partial z_1}v\enspace.
\end{equation}

The forward mode consists in doing the multiplications in \eqref{eq:chain_rule_jvp} from the right to the left. This corresponds to a forward pass in the computational graph where we propagate at the same time the states $$z_i$$ and the partial derivatives $$\frac{\partial z_{i+1}}{\partial z_i}$$. In the case where $$f$$ is real-valued, we obtain the gradient by computing $$d$$ JVPs since 

$$
\frac{\partial f}{\partial \theta_i}(\theta) = \frac{\partial f}{\partial \theta}(\theta) e_i
$$

where $$e_i$$ is the $$i$$-th canonical vector of $$\mathbb{R}^d$$.
...
In comparison with the code of the evaluation of $$f_x$$, there are two more operations corresponding to the computation of the dual variable `vz1` and `vz2`. In terms of memory, if we consider the computational of the JVP as coded in the previous snippet, the maximum number of parents of a vertex is four. This maximum is achieved by the vertex `vz2` which has the vertices `U`, `vz1`, `vu` and `z1` as parents.

In `JAX`, we get the JVP of $$f$$ at $$x$$ in the direction $$v$$ with `jax.jvp(f, (params, ), (v, ))[1]`.

### Reverse mode
The reverse mode aims at computing VJPs

\begin{equation}\label{eq:chain_rule_vjp}
  v^\top\frac{\partial f}{\partial \theta}(\theta) = v^\top\frac{\partial z_n}{\partial z_{n-1}}\frac{\partial z_{n-1}}{\partial z_{n-2}}\cdots\frac{\partial z_2}{\partial z_1}\enspace.
\end{equation}

In the reverse AD, we do the multiplications of \eqref{eq:chain_rule_jvp} from the left to the right. It requires doing one forward pass in the computational graph to compute the intermediate states $$z_i$$ and then a backward pass to propagate the successive partial derivatives from the left to the right. Contrary to the forward mode, it has a more important memory footprint. Indeed, it requires storing the values of all the states. For instance, to compute the last term $$\frac{\partial z_2}{\partial z_1}$$, one needs the value of $$z_1$$ which was the first computed during the forward pass. If $$f$$ is scalar, its gradient is a VJP with $$v=1$$. Thus, one can get the gradient on $$f$$ by performing only one reverse differentiation. This makes this mode more efficient to compute gradients.
...
Since we are interested in computing $$\nabla^2 f(x)v$$, the simplest way to do it is to compute the Hessian matrix and then multiply it by the vector $$v$$. This can be achieved in `JAX` by calling `jax.hessian(f)(params) @ v`.

This method is quite cumbersome making it impossible to use for deep neural networks. Indeed, the storage of the full Hessian matrix has $$\mathcal{O}(d^2)$$ complexity where $$d$$ is the input dimension.

However, it is possible to compute the HVP without computing the full Hessian matrix. 


## HVPs without explicit Hessian computation
A clever use of AD enables computing HVPs without computing the Hessian. The [JAX documentation](https://jax.readthedocs.io/en/latest/notebooks/autodiff_cookbook.html) describes three ways to do it.

The HVP is also the directional derivative of the gradient in the direction $$v$$:

$$
\nabla^2f(\theta) v = \lim_{\epsilon\to 0} \frac1\epsilon[\nabla f(\theta+\epsilon v)-\nabla f(\theta)] = \nabla [\langle \nabla f(.), v\rangle](\theta)\enspace.
$$

Pearlmutter<d-cite key="Pearlmutter1994"></d-cite> proposed to leverage this identity to compute HVP without computing Hessian. This can be done in three fashions: the forward-over-reverse, the reverse-over-reverse and the reverse-over-forward.
...
The take-home message of this part is that, after computing the gradient of $$f_x$$, one can consider a computational graph of this gradient and perform forward differentiation through this new computational graph. Here, the variables `z1`,..., `z6` are the vertices of a computational graph of the gradient of $$f_x$$. The nice thing is that the mode enables getting at the same time the gradient and the HVP. Indeed, in the previous snippet, `z4` and `z6` are actually the gradient of $$f_x$$ which could be also returned if needed.

### Reverse-over-reverse
Instead of doing forward differentiation of the gradient, one can multiply the gradient by $$v$$. Since we get a scalar, one can backpropagate in this scalar product. This is the reverse-over-reverse mode.
...
def reverse_over_reverse(U, W, vu, vw):
    ...
    z1 = W @ x
    z2 = U @ z1
    output = 0.5 * jnp.linalg.norm(z2)**2
...
However, we do the same experiment by *disabling the JIT compilation*. The result we get corroborates more the theory. Indeed, one can observe in the following figure that the memory footprint of the `reverse-over-reverse` method is larger than the one of the `forward-over-reverse` and `reverse-over-forward` methods. This is because the `reverse-over-reverse` involves two successives backward differentiations while the two others involve only one reverse differentiation. Moreover, it scales linearly with the batch size, which was not the case in the previous figure in the small batch size regime. In light of these two results, the clever memory allocation performed during just-in-time compilation reduces the memory footprint of the computations.

{% include figure.html path="assets/img/2024-05-07-bench-hvp/bench_hvp_memory_jax_without_jit.png" class="img-fluid" %}

In the following figure, we plot the results we get with the `PyTorch` implementation. One can observe that in all the cases the `forward-over-reverse` consumes much memory in comparison with the `reverse-over-forward` mode. Actually, it is almost at the same level as `reverse-over-reverse` mode. The right plot of the evolution of the memory footprint with the batch size for the ResNet34 architecture evolves linearly as expected.

{% include figure.html path="assets/img/2024-05-07-bench-hvp/bench_hvp_memory_torch.png" class="img-fluid" %}

## Conclusion

In this blogpost, we have explored the different ways to compute HVP from theoretical and practical perspectives. We end up with the following take-home messages:
