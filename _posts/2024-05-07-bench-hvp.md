...
To explain how AD works, we take a simple example. Let us consider a function $$f:\mathbb{R}^d\to\mathbb{R}^p$$ with this computational graph.
...
The take-home message of this part is that, after computing the gradient of $$f_x$$, one can consider a computational graph of this gradient and performs forward differentiation through this new computational graph. Here, the variables `z1`,..., `z6` are the vertices of a computational graph of the gradient of $$f_x$$. The nice thing is that this mode enables getting at the same time the gradient and the HVP. Indeed, in the previous snippet, `z4` and `z6` are actually the components of the gradient of $$f_x$$ which could be also returned if needed. This feature can be useful in bilevel optimization for instance.
...
Now we turn to the main contribution of this blog post, which is a benchmark of the different methods to compute HVPs.
...
In the following figure, we plot the results we get with the `PyTorch` implementation. One can observe that in all the cases the `forward-over-reverse` consumes much memory in comparison with the `reverse-over-forward` mode. It is almost at the same level as `reverse-over-reverse` mode, which is quite unexpected.
